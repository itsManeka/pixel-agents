...
/** Pre-compute set of tile keys that are blocked but belong to furniture with backgroundTiles > 0 */
export function buildBgFurnitureTiles(furniture: PlacedFurniture[]): Set<string> {
  const tiles = new Set<string>();
  for (const item of furniture) {
    const entry = getCatalogEntry(item.type);
    if (!entry || !entry.backgroundTiles) continue;
...
/** Check if a tile is walkable for pets — relaxed rules allow walking under furniture with backgroundTiles */
export function isPetWalkable(
  col: number,
  row: number,
  tileMap: TileType[][],
  blockedTiles: Set<string>,
  ...
  const t = tileMap[row][col];
  if (t === TileType.WALL || t === TileType.VOID) return false;
  if (!blockedTiles.has(`${col},${row}`)) return true;
  return bgFurnitureTiles.has(`${col},${row}`);
}

/** Get walkable tile positions for pets (superset of character walkable tiles) */
export function getPetWalkableTiles(
  tileMap: TileType[][],
  blockedTiles: Set<string>,
  furniture: PlacedFurniture[],
): Array<{ col: number; row: number }> {
  const bgTiles = buildBgFurnitureTiles(furniture);
  const rows = tileMap.length;
  const cols = rows > 0 ? tileMap[0].length : 0;
  const tiles: Array<{ col: number; row: number }> = [];
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (isPetWalkable(c, r, tileMap, blockedTiles, bgTiles)) {
        tiles.push({ col: c, row: r });
      }
    }
  }
...
/** BFS pathfinding for pets — uses isPetWalkable with pre-computed bgFurnitureTiles */
export function findPetPath(
  ...
  if (!isPetWalkable(endCol, endRow, tileMap, blockedTiles, bgFurnitureTiles)) {
    return [];
  }
  ...
  while (queue.length > 0) {
    ...
    for (const d of dirs) {
      ...
      if (visited.has(nk)) continue;
      if (!isPetWalkable(nc, nr, tileMap, blockedTiles, bgFurnitureTiles)) continue;
