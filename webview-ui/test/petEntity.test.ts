/**
 * Unit tests for Pet entity: createPet, updatePet FSM, isPetWalkable, findPetPath.
 *
...
import { findPetPath, getPetWalkableTiles, isPetWalkable } from '../src/office/layout/tileMap.js';
...
/** Empty bgFurnitureTiles set (no furniture with backgroundTiles) */
const noBgTiles = new Set<string>();

// ── createPet ───────────────────────────────────────────────

describe('createPet', () => {
  it('returns a Pet with correct initial values', () => {
    const pet = createPet('pet-1', 2, { col: 3, row: 4 });
...
describe('isPetWalkable', () => {
  const tileMap = makeTileMap(5, 5, [[0, 0]]);
  const blocked = new Set<string>();

  it('returns true for open floor tile', () => {
    assert.ok(isPetWalkable(1, 1, tileMap, blocked, noBgTiles));
  });

  it('returns false for WALL tile', () => {
    assert.ok(!isPetWalkable(0, 0, tileMap, blocked, noBgTiles));
  });

  it('returns false for out-of-bounds tile', () => {
    assert.ok(!isPetWalkable(-1, 0, tileMap, blocked, noBgTiles));
    assert.ok(!isPetWalkable(0, 5, tileMap, blocked, noBgTiles));
  });

  it('returns false for blocked tile with no bgFurniture', () => {
    const blockedSet = new Set(['2,2']);
    assert.ok(!isPetWalkable(2, 2, tileMap, blockedSet, noBgTiles));
  });

  it('returns true for blocked tile that has bgFurniture', () => {
    const blockedSet = new Set(['2,2']);
    const bgTiles = new Set(['2,2']);
    assert.ok(isPetWalkable(2, 2, tileMap, blockedSet, bgTiles));
  });
});

// ── getPetWalkableTiles ─────────────────────────────────────

describe('getPetWalkableTiles', () => {
  it('returns all floor tiles when none are blocked', () => {
    const tileMap = makeTileMap(3, 3);
    const tiles = getPetWalkableTiles(tileMap, new Set(), []);
    assert.equal(tiles.length, 9);
  });

  it('excludes WALL tiles', () => {
    const tileMap = makeTileMap(3, 3, [[0, 0], [1, 1]]);
    const tiles = getPetWalkableTiles(tileMap, new Set(), []);
...
describe('findPetPath', () => {
  ...
  it('routes through bgFurniture tiles when blocked', () => {
    const blocked = new Set(['2,0', '2,1', '2,2', '2,3', '2,4']);
    // Without bgTiles: no path possible (wall from row 0 to 4 at col 2)
    const pathBlocked = findPetPath(0, 0, 4, 0, tileMap, blocked, noBgTiles);
    assert.equal(pathBlocked.length, 0);
    // With bgTiles at col 2: pet can cross
    const bgTiles = new Set(['2,0', '2,1', '2,2', '2,3', '2,4']);
    const pathOpen = findPetPath(0, 0, 4, 0, tileMap, blocked, bgTiles);
    assert.ok(pathOpen.length > 0);
    assert.deepEqual(pathOpen[pathOpen.length - 1], { col: 4, row: 0 });
  });
});
...
describe('updatePet FSM', () => {
  const tileMap = makeTileMap(5, 5);
  const blocked = new Set<string>();
  const walkableTiles = getPetWalkableTiles(tileMap, blocked, []);
  const emptyChars = new Map<number, Character>();

  it('stays IDLE while wanderTimer > 0', () => {
    const pet = createPet('p1', 0, { col: 2, row: 2 });
    pet.wanderTimer = 5.0;
    updatePet(pet, 1.0, walkableTiles, emptyChars, tileMap, blocked, noBgTiles);
    assert.equal(pet.state, PetState.IDLE);
    assert.ok(pet.wanderTimer < 5.0);
  });

  it('transitions IDLE → WALK when wanderTimer expires', () => {
    const pet = createPet('p2', 0, { col: 2, row: 2 });
  ...
  it('transitions WALK → IDLE when path ends', () => {
    const pet = createPet('p3', 0, { col: 2, row: 2 });
    pet.state = PetState.WALK;
    pet.path = [{ col: 3, row: 2 }];
    pet.moveProgress = 0;
    updatePet(pet, 10.0, walkableTiles, emptyChars, tileMap, blocked, noBgTiles);
    assert.equal(pet.state, PetState.IDLE);
    assert.equal(pet.tileCol, 3);
  ...
  it('transitions FOLLOW → IDLE when target character is gone', () => {
    const pet = createPet('p4', 0, { col: 2, row: 2 });
    ...
    updatePet(pet, 0.1, walkableTiles, emptyChars, tileMap, blocked, noBgTiles);
    assert.equal(pet.state, PetState.IDLE);
    assert.equal(pet.followTargetId, null);
  });
  ...
  it('transitions FOLLOW → IDLE when already adjacent to target', () => {
    ...
    const pet = createPet('p5', 0, { col: 2, row: 2 });
    pet.state = PetState.FOLLOW;
    pet.followTargetId = 1;
    pet.followRecalcTimer = 0;
    pet.followDuration = 0;
    pet.followDurationLimit = PET_FOLLOW_DURATION_MAX_SEC;
    updatePet(pet, 0.1, walkableTiles, chars, tileMap, blocked, noBgTiles);
    assert.equal(pet.state, PetState.IDLE);
    assert.equal(pet.followTargetId, null);
  });
  ...
  it('transitions FOLLOW → IDLE when follow duration limit exceeded', () => {
    ...
    const pet = createPet('p6', 0, { col: 0, row: 0 });
    pet.state = PetState.FOLLOW;
    pet.followTargetId = 1;
    pet.followRecalcTimer = 5.0;
    pet.followDuration = 14.9;
    pet.followDurationLimit = 15.0;
    updatePet(pet, 0.2, walkableTiles, chars, tileMap, blocked, noBgTiles);
    assert.equal(pet.state, PetState.IDLE);
    assert.equal(pet.followTargetId, null);
  });
});
...
describe('getPetSpriteData', () => {
  it('returns null when petSprites is null', () => {
    const pet = createPet('p1', 0, { col: 0, row: 0 });
