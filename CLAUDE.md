...
## Pet Sprites & Animation

**Spritesheet format** (96×96 pixels, 3 rows of 32px height):

- **Row 1 (down/front)**: 6 frames of 16×32 — frames 0-2 walk (F1, F2, F3), frames 3-5 idle (F4, F5, F6)
- **Row 2 (up/back)**: 6 frames of 16×32 — frames 0-2 walk (F1, F2, F3), frames 3-5 idle (F4, F5, F6)
- **Row 3 (right)**: 3 frames of 32×32 — walk only (F1, F2, F3); left is horizontal flip at runtime
- **Idle**: Now animated (array of 3 frames), not a single static frame

**Animation sequences** (`webview-ui/src/constants.ts`):

- **Walk**: `[0, 1, 0, 2]` — 4-step cycle over 3 walk frames, reusing frame 0 twice
- **Idle**: `[0, 1, 2, 1]` — 4-step cycle over 3 idle frames, reusing frame 1 twice
- **Timings**: walk `0.15s/frame`, idle `0.3s/frame` (idle is slower)
...
**Direction mapping** (`webview-ui/src/office/sprites/petSpriteData.ts`, `setPetTemplates()`):

- `idleRight = idleDown` (frames from row 1, front)
- `idleLeft = idleUp` (frames from row 2, back)
- `walkLeft = horizontal flip of walkRight` (3 frames, not 2)

**Animation FSM** (`webview-ui/src/office/engine/petEntity.ts`):

- `pet.frame` is an index into the sequence, not a direct sprite index
- `updateWalkAnimation()`: increments `frameTimer`, wraps `frame` with `PET_WALK_SEQUENCE.length`
- `updateIdleAnimation()`: increments `frameTimer`, wraps `frame` with `PET_IDLE_SEQUENCE.length`
- `getPetSpriteData()` uses `PET_WALK_SEQUENCE[pet.frame]` and `PET_IDLE_SEQUENCE[pet.frame]` to index the actual sprite frames
